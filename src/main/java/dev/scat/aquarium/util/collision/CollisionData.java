...
import dev.scat.aquarium.util.Pair;
...
public enum CollisionData {
    // I will recode these grim blocks soon :)

    // TODO: update to modern versions
    ...
    CAKE((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        ...
        return new SimpleCollisionBox(f, 0, 0.0625F, 1 - 0.0625F,
                data.getVersion().isNewerThanOrEquals(ClientVersion.V_1_8)
    ...
    CHEST((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        ...
        WrappedBlockState northBlock = data.getWorldProcessor().getBlock(x, y, z - 1);
        WrappedBlockState southBlock = data.getWorldProcessor().getBlock(x, y, z + 1);
        WrappedBlockState eastBlock = data.getWorldProcessor().getBlock(x + 1, y, z);
        WrappedBlockState westBlock = data.getWorldProcessor().getBlock(x - 1, y, z);
    ...
    DOOR((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        ...
        if (block.getHalf() == Half.UPPER) {
            WrappedBlockState below = data.getWorldProcessor().getBlock(x, y - 1, z);
            ...
            WrappedBlockState above = data.getWorldProcessor().getBlock(x, y + 1, z);
            open = block.isOpen();
    ...
    FENCE((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        boolean north = BlockUtil.canConnectToFence(block, data.getWorldProcessor().getBlock(x, y, z - 1));
        boolean south = BlockUtil.canConnectToFence(block, data.getWorldProcessor().getBlock(x, y, z + 1));
        boolean east = BlockUtil.canConnectToFence(block, data.getWorldProcessor().getBlock(x + 1, y, z));
        boolean west = BlockUtil.canConnectToFence(block, data.getWorldProcessor().getBlock(x - 1, y, z));
    ...
    PANE((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        boolean north = BlockUtil.connectsToGlassPane(data.getWorldProcessor().getBlock(x, y, z - 1));
        boolean south = BlockUtil.connectsToGlassPane(data.getWorldProcessor().getBlock(x, y, z + 1));
        boolean east = BlockUtil.connectsToGlassPane(data.getWorldProcessor().getBlock(x + 1, y, z));
        boolean west = BlockUtil.connectsToGlassPane(data.getWorldProcessor().getBlock(x - 1, y, z));
    ...
    WALL((PlayerData data, ClientVersion version, WrappedBlockState block, int x, int y, int z) -> {
        boolean north = BlockUtil.canConnectToWall(data.getWorldProcessor().getBlock(x, y, z - 1));
        boolean south = BlockUtil.canConnectToWall(data.getWorldProcessor().getBlock(x, y, z + 1));
        boolean east = BlockUtil.canConnectToWall(data.getWorldProcessor().getBlock(x + 1, y, z));
        boolean west = BlockUtil.canConnectToWall(data.getWorldProcessor().getBlock(x - 1, y, z));
        float f = 0.25F;
        float f1 = 0.75F;
        float f2 = 0.25F;
        float f3 = 0.75F;
